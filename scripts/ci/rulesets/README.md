...
copy — someone still has to push it to the live ruleset via the API.
...
actionlint and by 39/39 real CI runs failing to even schedule a job on the
upstream gracefully-giving-back copy of this workflow, which tries exactly
that). `rulesets_drift_guard.yml` here mints a token from the App already
...
Vendored from the established Datalena/GGB implementation, following the
release-notes vendoring precedent. Keep
...
Unit tests are colocated at `scripts/ci/rulesets/test_rulesets_sync.py` so the
vendored guard and its regression coverage move together.
